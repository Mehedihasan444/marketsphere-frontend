...
const ProductInfo = ({ product }: { product: TProduct }) => {
  ...
  return (
    <div>
      <h3 className="">{product?.category?.name as string}</h3>
      <h2 className="text-2xl font-bold mb-2 mt-3">{product.name}</h2>
      ...
      <div className="mt-4 flex items-center space-x-4">
        <Button onClick={decreaseQuantity} disabled={quantity <= 1} icon="-" />
      ...
      <div className="mt-6 flex gap-5">
        ...
        <Button type="primary" danger size="large" className="flex-1">
          Add to Wishlist
        </Button>
      </div>
      <div className=""></div>
